...
function voyage_comment_prep() {
	return '<span class="meta-comment-prep">' . get_voyage_icon('comment') . '</span>';
}
...
if ( ! function_exists( 'get_voyage_icon' ) ) :
/* return back icon img tag */
function get_voyage_icon( $name ) {
	$str = '<img class="voyage-icon" src="';
	$str = $str . get_template_directory_uri()	. '/images/' . $name . '.png';
	$str = $str . '" alt="' . $name . '">';
	return $str;
}
endif;
...
function voyage_scripts_method() {	
	...
	wp_enqueue_style('Berkshire+Swash', "//fonts.googleapis.com/css?family=Berkshire+Swash", false, '1.0');
	...
	if ($voyage_options['mobile_style'] == "1" ) {
		...
		wp_enqueue_style('voyage-responsive', $theme_uri . '/css/responsive.min.css', array( 'voyage'), '1.2.6');
	}
